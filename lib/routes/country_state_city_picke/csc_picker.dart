...
class CSCPicker extends StatefulWidget {
  ...
  const CSCPicker({
    ...
    this.lgaSearchPlaceholder = "Search LGA",
  ...
  final String lgaSearchPlaceholder;
...
class CSCPickerState extends State<CSCPicker> {
  ...
  Future<void> setDefaults() async {
    ...
    if (widget.currentState != null &&
        widget.currentCountry == CscCountry.Nigeria.name) {
      //for now lga is only for Nigeria country
      setState(() => _selectedLga = widget.currentLga!); //
      await getNgnLGA();
    }
  ...
  ///get L.G.A from json response if Nigeria country is selected
  Future<List<String?>> getNgnLGA() async {
    _lga.clear();
    var response = await getNgnStateLGAResponse() as Map<String, dynamic>;

    response.forEach((key, value) {
      if (_selectedState.toLowerCase().contains(key)) {
        var takeLGA = response[key]['locals']
            .map((item) => Lga.fromJson(item).name)
            .toList();
        var lga = takeLGA as List;
        for (var element in lga) {
          if (!mounted) continue;
          // print(element);
          setState(() {
            _lga.add(element);
  ...
  void _onSelectedCountry(String value) {
    ...
    setState(() {
      ...
      if (value != _selectedCountry) {
        ...
        _selectedState = widget.stateDropdownLabel;
        _selectedCity = widget.cityDropdownLabel;
        _selectedLga = widget.lgaDropdownLabel;
        widget.onStateChanged!(null);
        widget.onCityChanged!(null);
        widget.onLgaChanged!(null);
        ...
        widget.onLgaChanged!(_selectedLga);
      }
    });
  }
  ...
  void _onSelectedLga(String value) {
    ...
    setState(() {
      //code added in if condition
      if (value != _selectedLga) {
        _selectedLga = value;
        widget.onLgaChanged!(value);
      }
    });
  }
        ...
        widget.layout == Layout.vertical
            ? Column(
                ...
                children: <Widget>[
                  ...
                  if (widget.showStates == true &&
                      widget.showLga == true &&
                      _selectedCountry.contains(CscCountry.Nigeria.name)) ...[
                    Row(
                      children: [
                        Text(
            ...
            : Column(
                children: [
                  ...
                  widget.showStates &&
                          widget.showLga &&
                          _selectedCountry == CscCountry.Nigeria.name
                      ? lagDropdown()
                      : Container()
                ],
              ),
  ...
  Widget lagDropdown() {
    return DropdownWithSearch(
      title: widget.lgaDropdownLabel,
      placeHolder: widget.lgaSearchPlaceholder,
      disabled: _lga.length == 0 ? true : false,
      items: _lga.map((String? dropDownStringItem) {
        return dropDownStringItem;
      }).toList(),
      selectedItemStyle: widget.selectedItemStyle,
      dropdownHeadingStyle: widget.dropdownHeadingStyle,
      ...
      label: widget.lgaSearchPlaceholder,
      onChanged: (value) {
        //print("lgaChanged $value $_selectedLga");
        value != null ? _onSelectedLga(value) : _onSelectedLga(_selectedLga);
      },
    );
  }
}
